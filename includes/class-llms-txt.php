...
class GetCited_Llms_Txt {
    ...
    /**
     * Assess an existing llms.txt file to determine its value/effort level
     *
     * @param string $content The llms.txt content to assess.
     * @return string 'basic', 'moderate', or 'substantial'
     */
    public function assess_llms_txt( $content ) {
        // Self-declared auto-generated files → always basic
        if ( preg_match( '/generated by/i', $content ) ) {
            return 'basic';
        }

        // Otherwise use length as proxy for effort
    ...
    /**
     * Import existing llms.txt content into GetCited management
     *
     * @return array Result with success status and message.
     */
    public function import_existing_file() {
        $content = $this->get_physical_file_content();

        if ( empty( $content ) ) {
            return array(
                'success' => false,
                'message' => __( 'No existing llms.txt file found to import.', 'getcited' ),
            );
        }
        ...
        // Write the file with GetCited marker
        $this->write_physical_file();
