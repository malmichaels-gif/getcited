...
class GetCited_Llms_Txt {
     ...
     *
     * @param string|null $content Optional content to write. If null, reads from settings.
     * @return array Result with success status and message
     */
    public function write_physical_file( $content = null ) {
        global $wp_filesystem;
        ...
        // Get content from parameter or settings
        if ( null === $content ) {
    ...
    /**
     * Backup existing physical llms.txt file to llms.txt.bak
     *
     * Used when user switches to GetCited-managed llms.txt, preserving their original.
     *
     * @since 1.9.9.18
     * @return array Result with success status and message.
     */
    public function backup_physical_file() {
        $file_path   = ABSPATH . 'llms.txt';
        ...
        if ( ! $wp_filesystem->exists( $file_path ) ) {
            return array(
                'success' => true,
                'message' => __( 'No llms.txt file to backup.', 'getcited' ),
            );
        }

        // Don't backup our own files.
        if ( $this->is_our_physical_file() ) {
            // Just delete it instead - no need to backup GetCited-generated files.
            return $this->delete_physical_file();
        }

        // Move file to backup (overwrites any existing .bak).
        $result = $wp_filesystem->move( $file_path, $backup_path, true );

        if ( ! $result ) {
            return array(
                'success' => false,
                'message' => __( 'Failed to backup llms.txt file.', 'getcited' ),
            );
        }

        return array(
            'success' => true,
            'message' => __( 'llms.txt backed up to llms.txt.bak', 'getcited' ),
        );
    }

    /**
     * Restore llms.txt.bak to llms.txt
     *
     * Used when user switches back to their original file or on uninstall.
     *
     * @since 1.9.9.18
     * @return array Result with success status and message.
     */
    public function restore_physical_file() {
        $file_path   = ABSPATH . 'llms.txt';
        $backup_path = ABSPATH . 'llms.txt.bak';
        ...
        if ( ! $wp_filesystem->exists( $backup_path ) ) {
            return array(
                'success' => false,
                'message' => __( 'No backup file found.', 'getcited' ),
            );
        }
        ...
        $result = $wp_filesystem->move( $backup_path, $file_path, true );

        if ( ! $result ) {
            return array(
                'success' => false,
                'message' => __( 'Failed to restore llms.txt from backup.', 'getcited' ),
            );
        }

        return array(
            'success' => true,
            'message' => __( 'llms.txt restored from backup', 'getcited' ),
        );
    }
    ...
    public function backup_exists() {
        return file_exists( ABSPATH . 'llms.txt.bak' );
    }

    /**
     * Detect which SEO plugin created an llms.txt file
     *
     * Returns the SEO plugin name if detected, false otherwise.
     * This is used to show specific conflict warnings with disable instructions.
     *
     * @since 1.9.9.19
     * @param string $content The llms.txt content to analyze.
     * @return string|false Plugin identifier ('yoast', 'rankmath', 'aioseo', 'squirrly') or false.
     */
    public function detect_seo_plugin_source( $content ) {
        if ( empty( $content ) ) {
            return false;
        }
        ...
        // Yoast SEO
        if (
            strpos( $content_lower, 'yoast' ) !== false ||
            strpos( $content_lower, 'generated by yoast' ) !== false
        ) {
            return 'yoast';
        }
        ...
        // RankMath
        if (
            strpos( $content_lower, 'rankmath' ) !== false ||
            strpos( $content_lower, 'rank math' ) !== false
        ) {
            return 'rankmath';
        }
        ...
        // All in One SEO
        if (
            strpos( $content_lower, 'aioseo' ) !== false ||
            strpos( $content_lower, 'all in one seo' ) !== false
        ) {
            return 'aioseo';
        }

        // Squirrly SEO
        if ( strpos( $content_lower, 'squirrly' ) !== false ) {
            return 'squirrly';
        }

        // SEOPress
        if ( strpos( $content_lower, 'seopress' ) !== false ) {
            return 'seopress';
        }
    ...
    /**
     * Get disable instructions for a specific SEO plugin
     *
     * @since 1.9.9.19
     * @param string $plugin Plugin identifier from detect_seo_plugin_source().
     * @return array Array with 'name' and 'instructions' keys.
     */
    public function get_seo_plugin_disable_instructions( $plugin ) {
        $instructions = array(
            'yoast' => array(
                'name'         => 'Yoast SEO',
                'instructions' => __( 'Go to Yoast SEO → Settings → Site Features → APIs → disable "Enable llms.txt"', 'getcited' ),
            ),
            'rankmath' => array(
                'name'         => 'Rank Math',
                'instructions' => __( 'Go to Rank Math → General Settings → Others → disable "Enable llms.txt"', 'getcited' ),
            ),
            'aioseo' => array(
                'name'         => 'All in One SEO',
                'instructions' => __( 'Go to All in One SEO → Search Appearance → Advanced → disable llms.txt generation', 'getcited' ),
            ),
            'squirrly' => array(
                'name'         => 'Squirrly SEO',
                'instructions' => __( 'Go to Squirrly SEO → Settings and disable llms.txt feature', 'getcited' ),
            ),
            'seopress' => array(
                'name'         => 'SEOPress',
                'instructions' => __( 'Go to SEOPress → Settings and disable llms.txt feature', 'getcited' ),
            ),
        );

        return $instructions[ $plugin ] ?? array(
            'name'         => __( 'SEO Plugin', 'getcited' ),
            'instructions' => __( 'Check your SEO plugin settings and disable its llms.txt feature', 'getcited' ),
        );
    }

    /**
     * Check for active SEO plugin llms.txt conflicts
     *
     * Checks if a physical llms.txt exists that was created by an SEO plugin,
     * which would override GetCited's dynamic serving.
     *
     * @since 1.9.9.19
     * @return array|false Array with 'plugin' and 'instructions' if conflict found, false otherwise.
     */
    public function check_seo_plugin_conflict() {
        // No conflict if no physical file
        ...
        $plugin  = $this->detect_seo_plugin_source( $content );

        if ( ! $plugin ) {
            return false;
        }

        $info = $this->get_seo_plugin_disable_instructions( $plugin );
    ...
    private function output_llms_txt() {
        ...
        header( 'Cache-Control: no-cache, no-store, must-revalidate' ); // No CDN caching - file is tiny, users need to verify edits immediately
     ...
     *
     * As of v1.9.9.19, all llms.txt content is managed by GetCited.
     * Existing files are imported and backed up on activation.
     */
