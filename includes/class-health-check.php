...
class GetCited_Health_Check {
    ...
    private function check_llms_txt() {
        ...
        // Check for any physical file conflict (SEO plugin, legacy, or unknown)
        // Simplified to single "conflict" state in v1.9.9.19
        if ( $summary['physical_file_exists'] ) {
            ...
            // Check if it's a GetCited file (legacy - can be auto-deleted)
            if ( $content && strpos( $content, '# Generated by GetCited' ) !== false ) {
                // Try to auto-delete legacy files
                $llms->delete_physical_file();

                // If still exists after delete attempt, show error
                if ( $llms->physical_file_exists() ) {
                    return array(
                        'status' => 'error',
                        'message' => __( 'Could not remove old file', 'getcited' ),
                        'details' => __( 'An old GetCited file exists but cannot be deleted. Check file permissions.', 'getcited' ),
