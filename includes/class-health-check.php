...
class GetCited_Health_Check {
    ...
    private function check_llms_txt() {
        ...
        // Check for SEO plugin conflict first (v1.9.9.19)
        ...
        if ( $seo_conflict ) {
            return array(
                ...
                'message' => sprintf(
                    ...
                    __( '%s llms.txt conflict', 'getcited' ),
                    $seo_conflict['name']
                ),
                'details' => sprintf(
                    /* translators: 1: SEO plugin name, 2: Instructions to disable */
                    __( '%1$s is generating a physical llms.txt file that overrides GetCited. %2$s', 'getcited' ),
                    $seo_conflict['name'],
                    $seo_conflict['instructions']
                ),
                'seo_plugin' => $seo_conflict['plugin'],
                'action_type' => 'delete_conflict',
                'action_label' => __( 'Remove conflicting file', 'getcited' ),
            );
        }

        // Check for physical llms.txt file (not SEO plugin - could be old GetCited file or manual)
        if ( $summary['physical_file_exists'] ) {
            ...
            // Check if it's ours (legacy physical file)
            if ( $content && strpos( $content, '# Generated by GetCited' ) !== false ) {
                // Legacy physical file from older version - should be deleted
                return array(
                    'status' => 'warning',
                    'message' => __( 'Legacy physical llms.txt file found', 'getcited' ),
                    'details' => __( 'GetCited now serves llms.txt dynamically. The physical file can be removed.', 'getcited' ),
                    'action_type' => 'delete_physical',
                    'action_label' => __( 'Remove physical file', 'getcited' ),
                    'url' => home_url( '/llms.txt' ),
                );
            }

            // Physical file exists but not ours (unknown source)
            ...
            return array(
                ...
                'details' => $source
                    ? sprintf(
                        ...
                        __( 'A physical llms.txt file exists, possibly created by %s. This takes precedence over GetCited. Delete the physical file to use GetCited.', 'getcited' ),
                        $source
                    )
                    : __( 'A physical llms.txt file in your site root takes precedence over GetCited. Delete the physical file to use GetCited.', 'getcited' ),
                'file_path' => $file_path,
                'action_type' => 'delete_conflict',
                'action_label' => __( 'Remove conflicting file', 'getcited' ),
